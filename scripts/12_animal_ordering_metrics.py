#!/usr/bin/env python3
"""Evaluate imposed animal list order against random permutation baselines."""
...
from lce.concepts import ANIMAL_ORDERING_EVAL_MODES, ANIMAL_ORDERINGS
    ...
    for layer in layers:
        ...
        for sp in spaces:
            ...
            for ordering_name in orderings:
                if ordering_name not in ANIMAL_ORDERINGS:
                    raise KeyError(f"unknown ordering {ordering_name!r}")
                ordering = ANIMAL_ORDERINGS[ordering_name]
                ...
                eval_modes = ANIMAL_ORDERING_EVAL_MODES.get(ordering_name, (("default", False),))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Animal ordering topology metrics.")
    parser.add_argument("--config", type=Path, default=ROOT / "configs" / "animals_distilgpt2.json")
    ...
    output = (
        Path(args.output)
        if args.output
        else ROOT / cfg["output"]["metrics_dir"] / "animals_orderings.json"
    )
    orderings = cfg.get("topology", {}).get("orderings", list(ANIMAL_ORDERINGS.keys()))
