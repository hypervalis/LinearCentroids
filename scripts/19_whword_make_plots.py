#!/usr/bin/env python3
"""Generate Plotly PCA path plots for the whword list ordering."""
...
from lce.concepts import WHWORD_ORDERING_EVAL_MODES, WHWORD_ORDERINGS
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs" / "whwords_distilgpt2.json")
    ...
    orderings = cfg.get("topology", {}).get("orderings", list(WHWORD_ORDERINGS.keys()))
    ...
    if args.sync_docs:
        exp = cfg.get("experiment", "whwords_distilgpt2")
        ...
        metrics_src = ROOT / cfg["output"]["metrics_dir"] / "whwords_orderings.json"
        if metrics_src.is_file():
            shutil.copy2(metrics_src, ROOT / "docs" / "experiments" / exp / "whwords_orderings.json")
