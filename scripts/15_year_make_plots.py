#!/usr/bin/env python3
"""Generate Plotly PCA path plots for the year list ordering."""
...
from lce.concepts import YEAR_ORDERING_EVAL_MODES, YEAR_ORDERINGS
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs" / "years_distilgpt2.json")
    ...
    orderings = cfg.get("topology", {}).get("orderings", list(YEAR_ORDERINGS.keys()))
    ...
    if args.sync_docs:
        exp = cfg.get("experiment", "years_distilgpt2")
        ...
        metrics_src = ROOT / cfg["output"]["metrics_dir"] / "years_orderings.json"
        if metrics_src.is_file():
            shutil.copy2(metrics_src, ROOT / "docs" / "experiments" / exp / "years_orderings.json")
