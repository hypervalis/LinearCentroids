#!/usr/bin/env python3
"""Generate Plotly PCA path plots for the animal list ordering."""
...
def main() -> None:
    ...
    for layer in cfg["hook"]["layers"]:
        ...
        for space in ("activation", "centroid"):
            for ordering_name in orderings:
                ...
                for mode_name, cyclic in ANIMAL_ORDERING_EVAL_MODES[ordering_name]:
    ...
    if args.sync_docs:
        exp = cfg.get("experiment", "animals_distilgpt2")
        ...
        metrics_src = ROOT / cfg["output"]["metrics_dir"] / "animals_orderings.json"
        if metrics_src.is_file():
            shutil.copy2(metrics_src, ROOT / "docs" / "experiments" / exp / "animals_orderings.json")
