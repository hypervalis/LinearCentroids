#!/usr/bin/env python3
"""Generate Plotly PCA path plots for the language list ordering."""
...
def main() -> None:
    ...
    for layer in cfg["hook"]["layers"]:
        ...
        for space in ("activation", "centroid"):
            for ordering_name in orderings:
                ...
                for mode_name, cyclic in LANGUAGE_ORDERING_EVAL_MODES[ordering_name]:
    ...
    if args.sync_docs:
        exp = cfg.get("experiment", "languages_distilgpt2")
        ...
        metrics_src = ROOT / cfg["output"]["metrics_dir"] / "languages_orderings.json"
        if metrics_src.is_file():
            shutil.copy2(metrics_src, ROOT / "docs" / "experiments" / exp / "languages_orderings.json")
