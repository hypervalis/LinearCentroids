#!/usr/bin/env python3
"""Evaluate imposed colour orderings against random permutation baselines."""
...
from lce.concepts import COLOUR_ORDERING_EVAL_MODES, COLOUR_ORDERINGS
...
def _points_by_colour(raw: dict, space: str) -> dict[str, np.ndarray]:
    """Template-mean vector per colour label."""
    ...
    for col, label in enumerate(concepts[:n_concepts]):
        by_label[label] = np.mean(tensor[:, col, :], axis=0)
    return by_label
...
def run_ordering_metrics(
    *,
    ...
    for layer in layers:
        ...
        for sp in spaces:
            points_by_label = _points_by_colour(raw, sp)
            results["layers"][str(layer)][sp] = {}

            for ordering_name in orderings:
                if ordering_name not in COLOUR_ORDERINGS:
                    raise KeyError(f"unknown ordering {ordering_name!r}")
                ordering = COLOUR_ORDERINGS[ordering_name]
                ...
                eval_modes = COLOUR_ORDERING_EVAL_MODES.get(ordering_name, (("default", False),))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Colour ordering topology metrics.")
    parser.add_argument("--config", type=Path, default=ROOT / "configs" / "colours_distilgpt2.json")
    parser.add_argument("--raw-dir", type=Path, default=None, help="Override raw extraction directory")
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output JSON (default: results/metrics/colours_orderings.json or from config)",
    )
    parser.add_argument("--space", default="both", help="activation, centroid, or both")
    ...
    output = (
        Path(args.output)
        if args.output
        else ROOT / cfg["output"]["metrics_dir"] / "colours_orderings.json"
    )
    orderings = cfg.get("topology", {}).get("orderings", list(COLOUR_ORDERINGS.keys()))
