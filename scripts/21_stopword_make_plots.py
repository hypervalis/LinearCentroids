#!/usr/bin/env python3
"""Generate Plotly PCA path plots for the stopword list ordering."""
...
from lce.concepts import STOPWORD_ORDERING_EVAL_MODES, STOPWORD_ORDERINGS
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs" / "stopwords_distilgpt2.json")
    ...
    orderings = cfg.get("topology", {}).get("orderings", list(STOPWORD_ORDERINGS.keys()))
    ...
    if args.sync_docs:
        exp = cfg.get("experiment", "stopwords_distilgpt2")
        ...
        metrics_src = ROOT / cfg["output"]["metrics_dir"] / "stopwords_orderings.json"
        if metrics_src.is_file():
            shutil.copy2(metrics_src, ROOT / "docs" / "experiments" / exp / "stopwords_orderings.json")
