#!/usr/bin/env python3
"""Evaluate imposed noun list order against random permutation baselines."""
...
from lce.concepts import NOUN_ORDERING_EVAL_MODES, NOUN_ORDERINGS
    ...
    for layer in layers:
        ...
        for sp in spaces:
            ...
            for ordering_name in orderings:
                if ordering_name not in NOUN_ORDERINGS:
                    raise KeyError(f"unknown ordering {ordering_name!r}")
                ordering = NOUN_ORDERINGS[ordering_name]
                ...
                eval_modes = NOUN_ORDERING_EVAL_MODES.get(ordering_name, (("default", False),))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Noun ordering topology metrics.")
    parser.add_argument("--config", type=Path, default=ROOT / "configs" / "nouns_distilgpt2.json")
    ...
    output = (
        Path(args.output)
        if args.output
        else ROOT / cfg["output"]["metrics_dir"] / "nouns_orderings.json"
    )
    orderings = cfg.get("topology", {}).get("orderings", list(NOUN_ORDERINGS.keys()))
