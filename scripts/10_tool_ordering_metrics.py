#!/usr/bin/env python3
"""Evaluate imposed tool list order against random permutation baselines."""
    ...
    for layer in layers:
        ...
        for sp in spaces:
            ...
            for ordering_name in orderings:
                if ordering_name not in TOOL_ORDERINGS:
                ...
                eval_modes = TOOL_ORDERING_EVAL_MODES.get(ordering_name, (("default", False),))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tool ordering topology metrics.")
    ...
    output = (
        Path(args.output)
        if args.output
        else ROOT / cfg["output"]["metrics_dir"] / "tools_orderings.json"
    )
