#!/usr/bin/env python3
"""Generate Plotly PCA path plots for each colour ordering."""
...
def _points_by_colour(raw: dict) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
...
def main() -> None:
    ...
    parser.add_argument("--sync-docs", action="store_true", help="Copy plots into docs/experiments/")
    ...
    for layer in cfg["hook"]["layers"]:
        ...
        act_by_label, cen_by_label = _points_by_colour(raw_dict)
        ...
        for space in ("activation", "centroid"):
            for ordering_name in orderings:
                ...
                for mode_name, cyclic in COLOUR_ORDERING_EVAL_MODES[ordering_name]:
    ...
    if args.sync_docs:
        exp = cfg.get("experiment", "colours_distilgpt2")
        ...
        metrics_src = ROOT / cfg["output"]["metrics_dir"] / "colours_orderings.json"
        if metrics_src.is_file():
            shutil.copy2(metrics_src, ROOT / "docs" / "experiments" / exp / "colours_orderings.json")
