#!/usr/bin/env python3
"""Evaluate imposed year list order against random permutation baselines."""
    ...
    for layer in layers:
        ...
        for sp in spaces:
            ...
            for ordering_name in orderings:
                if ordering_name not in YEAR_ORDERINGS:
                ...
                eval_modes = YEAR_ORDERING_EVAL_MODES.get(ordering_name, (("default", False),))
...
def main() -> None:
    ...
    output = (
        Path(args.output)
        if args.output
        else ROOT / cfg["output"]["metrics_dir"] / "years_orderings.json"
    )
