#!/usr/bin/env python3
"""Evaluate imposed stopword list order against random permutation baselines."""
    ...
    for layer in layers:
        ...
        for sp in spaces:
            ...
            for ordering_name in orderings:
                if ordering_name not in STOPWORD_ORDERINGS:
                ...
                eval_modes = STOPWORD_ORDERING_EVAL_MODES.get(ordering_name, (("default", False),))
...
def main() -> None:
    ...
    output = (
        Path(args.output)
        if args.output
        else ROOT / cfg["output"]["metrics_dir"] / "stopwords_orderings.json"
    )
