#!/usr/bin/env python3
"""Generate Plotly PCA path plots for the discipline list ordering."""
...
from lce.concepts import DISCIPLINE_ORDERING_EVAL_MODES, DISCIPLINE_ORDERINGS
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs" / "disciplines_distilgpt2.json")
    ...
    orderings = cfg.get("topology", {}).get("orderings", list(DISCIPLINE_ORDERINGS.keys()))
    ...
    for layer in cfg["hook"]["layers"]:
        ...
        for space in ("activation", "centroid"):
            for ordering_name in orderings:
                ordering = DISCIPLINE_ORDERINGS[ordering_name]
                if any(label not in act_by_label for label in ordering):
                    print(f"skip {ordering_name}: missing labels in layer {layer}")
                    continue

                for mode_name, cyclic in DISCIPLINE_ORDERING_EVAL_MODES[ordering_name]:
    ...
    if args.sync_docs:
        exp = cfg.get("experiment", "disciplines_distilgpt2")
        ...
        metrics_src = ROOT / cfg["output"]["metrics_dir"] / "disciplines_orderings.json"
        if metrics_src.is_file():
            shutil.copy2(metrics_src, ROOT / "docs" / "experiments" / exp / "disciplines_orderings.json")
